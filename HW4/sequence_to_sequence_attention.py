...
training_log_file = open('graphs/seq_to_seq_attention/Sequence_to_sequence_prediction_attention_'+ str(time.time()) +'.log', 'w')

# Save parameters to log
training_log_file.write(f'Sequence to sequence prediction with attention' + '\n\n')
...
def train(input, target):
    ...
    encoder_outputs = torch.zeros(hidden_size, hidden_size, device=device)
   ...
   # Decoder starts with the encoder's last hidden state
    ...
    for di in range(target_length-1):
        ...
        if(engToFren):
            ...
            if decoder_input.item() == dataset.fr_vocab.word2index['<SOS>']:  # Stop if EOS token is generated
                break
...
plt.legend()
plt.savefig('./graphs/seq_to_seq_attention/Sequence_to_sequence_prediction_attention.png')
plt.close()

print("Training Loss Graph Generated")

# save Model
torch.save(encoder, 'model/with_attention/encoder_sequence_to_sequence_prediction.pt')
torch.save(decoder, 'model/with_attention/decoder_sequence_to_sequence_prediction.pt')
print('Model saved')
